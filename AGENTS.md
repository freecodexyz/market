...
- `src/IDopplerHookInitializer.sol`: the Doppler initializer boundary the splitter collects fees through, plus the Uniswap V4 `PoolKey` needed to derive a pool id.
...
- `src/ClaimMatcher.sol`: the claim matcher `RIK` links against. Same semantics as `JsonClaim`, with a resumable Boyer-Moore-Horspool scan.
...
- `register-rik.yml` simulates with `cast call` before `cast send`. This prevents an already-registered repository, an unsynced signing key or an expired token from costing gas, and limits the ability to grief the registrar key.
...
- `ClaimMatcher.indexOf` and `JsonClaim.indexOf` are inline assembly. They deliberately read up to 31 bytes past the end of both arrays and mask them off, and never write; do not add the `memory-safe` annotation on the strength of that. Verify any edit differentially against both `JsonClaim` and the naive byte-at-a-time search before trusting it.
- `ClaimMatcher.indexOfFrom` resumes at a caller-supplied cursor and wraps to the beginning, so it still inspects every candidate position; only *which* occurrence it reports changes, and `indexOf` pins the from-zero case. `RIK` checks its claims in the order GitHub emits them and threads the cursor through, which turns five passes over the payload into one. Correctness never depends on that order — a claim behind the cursor is found on the wrap — so reordering the checks is a gas decision, not a safety one. It does decide which claim reports a failure first, and the negative tests pin that.
- `ClaimMatcher` skips with Boyer-Moore-Horspool: a bitmap of the bytes occurring in the needle lets a haystack byte that appears nowhere in it skip a whole needle length. The bitmap is built by reading whole words, so it can pick up bytes past the end of the needle. That is safe in exactly one direction — extra bits only make the scan more cautious — and a bitmap that ever *missed* a needle byte would skip a real match. Never make it tighter by narrowing what goes in.
- `RIK.audienceOf` writes the address, both separators and both decimal numbers in one pass, and `RIK._decimalText` replaces `Strings.toString`. Both are `memory-safe`, unlike the matchers. They are justified only while they remain cheaper than the readable form; `test/RegisterProfile.t.sol` and `test/GasRegression.t.sol` assert this. If either assertion fails, remove the encoder rather than maintaining it.
- `RIK._addressText` is no longer called by anything in `src/`. It is the readable reference `audienceOf` is fuzzed against, solc drops it from the deployed bytecode, and `slither` is told so in place. If the tests that exercise it are ever repointed at `audienceOf`, delete it.
...
- `foundry.toml` also pins `solc`. A deployment must be reproducible from source later, and an unpinned compiler tracks the toolchain.
- Every static-analysis finding is either fixed or accepted in place with a `slither-disable-next-line` and a stated reason, mirrored in `SECURITY.md`. `slither .` must remain at zero findings, so a new finding requires an explicit decision.
- `forge lint` is held to the same standard and reads the same files: `[lint] ignore` in `foundry.toml` mirrors `filter_paths` in `slither.config.json`, so the vendored sources, `test/` and `script/` are out of scope for both. A warning in `src/` is fixed or carries a `forge-lint: disable-next-line` with a reason, mirrored in `SECURITY.md`. Use a `disable-start`/`disable-end` pair when the statement spans more than one line — a `disable-next-line` only covers a single-line span — or when a `slither-disable-next-line` has to keep the line directly above the statement; neither tool looks past an intervening comment.
- The lint ids those directives name arrived in Foundry 1.8. `test-contracts.yml` installs `stable` on purpose, so a local toolchain older than that reports them as unknown ids and fails `forge lint --deny warnings` on the directives themselves. Match CI rather than pinning the workflow.
...
- `RIK` mints once per repository. There is no rebinding path and must not be one: re-registration would let a repository owner reclaim a key that has been sold. Wallet rotation is done by transfer.
...
- `RIK` metadata interpolates numeric ids only, never a repository name or owner login. This is what makes the JSON injection-free without a charset validator. Adding an attacker-controlled string reintroduces the problem `UIK._requireRenderableLogin` exists to solve.
...
- `src/IAirlock.sol` was checked against the Airlock deployed at `0x660eAaEdEBc968f8f3694354FA8EC0b4c5Ba8D12` on Base Mainnet: all three selectors it declares are present in that bytecode, the `CreateParams` field order matches, and `integrator` is field 11, which is the one `RIKLauncher` overwrites. The suite only ever exercises a mock, so this interface is the seam nothing on-chain would catch. Re-check it against the deployed contract after any edit.
- `src/IDopplerHookInitializer.sol` was checked against `DopplerHookInitializer` and `RehypeDopplerHookInitializer` on Base Mainnet: `getState(address)`, `getShares(bytes32,address)` and `collectFees(bytes32)` are all present, and the seven-element `getState` tuple decodes against the live contract. The suite only exercises a mock, so re-check it after any edit; SECURITY.md records the selectors.
- Doppler releases a pool's fees only to the **caller's** registered share, and beneficiaries are fixed when the pool is created. `RIKRoyaltySplitter` must therefore make the `collectFees` call itself, and `RIKLauncher` rejects a launch in which the splitter is not a beneficiary, because that cannot be corrected afterwards and the repository would accrue nothing.
- A pool is a Uniswap V4 `PoolKey`, not a contract. Its id is `keccak256(abi.encode(poolKey))`, which is what `PoolIdLibrary.toId` computes. The key is read from the initializer using the asset; do not accept a pool id from a caller.
- Fees are released as ERC20 transfers, so `RIKLauncher` rejects a native numeraire. Supporting one would require the splitter to hold and account for native value.
...
- `MARKET_RPC_URL` and `MARKET_CHAIN_ID` override the recorded endpoint and pin the chain; `--rpc-url` and `--chain-id` do the same per command. The chain id is a guard rather than a setting: `deploy` and anything that spends gas refuse to run against a chain other than the one named, and reads are deliberately not gated. `smoke-test.sh` proves the refusal rather than assuming it.
- A testnet deployment requires its own verifier, because the `identity` instance is on Base Mainnet and `market` never deploys one. `deploy` rejects a verifier address with no code, so omitting this step fails immediately instead of producing a registry that rejects every proof.
- The helper never writes a private key to `.market.yml`. Keys come from the environment or a no-echo prompt on each run, which is why that file is safe to leave in a working tree.
...
Four stateful invariants are load-bearing and must keep passing. Do not weaken a handler to make one pass. A handler that stops reaching a path makes its invariant trivially true, which is why every campaign asserts its own call counters in `afterInvariant`.
...
New assembly, new external calls, and new arithmetic on value all need fuzz coverage rather than a single example. Prefer a differential test against an obviously-correct reference when replacing something that already works; `ClaimMatcher`, `RIK.audienceOf` and `RIK._decimalText` all exist only because they are faster, and each is held to the implementation it replaced.
